...
namespace Esint.CodeBuilder.Template
{
    public class DAL : ITemplate
    {
        ...
        public IReturnCode[] GetCode()
        {
            ...
            sb.AppendLine("using Esint."+NameSpace+".IDAL;");
            ...
            sb.AppendLine("    /// 作    者："+UserName);
            sb.AppendLine("    /// 生成日期：" + DateTime.Now.ToString("yyyy年MM月dd日"));
            sb.AppendLine("    /// </summary>");
            sb.AppendLine("    public class " + Tbl.PascalName + "Data  : BaseData, I" + Tbl.PascalName + "Data");
            sb.AppendLine("    {");
            ...
            sb.AppendLine("");
            sb.AppendLine("        /// <summary>");
            sb.AppendLine("        /// 根据List Wherepara条件删除数据，带事务和日志功能。");
            sb.AppendLine("        /// </summary>");
            sb.AppendLine("       public void Delete(List<WhereParameter> whereParaList, IDbTransaction trans)");
            sb.AppendLine("       {");
            sb.AppendLine("            //建立SQL语句对象");
            sb.AppendLine("            OperateObject operateObject = CreateDeleteObject(whereParaList);");
            sb.AppendLine("            base.ExecuteNonQuery(operateObject, trans);");
            ...
            sb.AppendLine("        #region 查询方法集");

            sb.AppendLine();
            sb.AppendLine("        #region 根据主键查询得到实体(事务版)");
            sb.AppendLine();
            sb.AppendLine("        /// <summary>");
            sb.AppendLine("        /// 根据主键得到一个实体(事务版)");
            sb.AppendLine("        /// </summary>");
            sb.AppendLine("        /// <returns></returns>");
            ...
            sb.AppendLine();
            sb.AppendLine("        #region 根据主键查询得到实体(非事务版)");
            sb.AppendLine();
            sb.AppendLine("        /// <summary>");
            sb.AppendLine("        /// 根据主键得到一个实体(非事务版)");
            sb.AppendLine("        /// </summary>");
            sb.AppendLine("        /// <returns></returns>");
            ...
            sb.AppendLine("            " + Tbl.PascalName + "Info " + Tbl.CamelName + "=null;");
            sb.AppendLine("                DbTrans trans = new DbTrans();");
            sb.AppendLine("                using (IDataReader dr = basicDataAccess.ExecuteReader(trans, CommandType.Text, strSql.ToString(), parameters))");
            sb.AppendLine("                {");
            sb.AppendLine("                    if (dr.Read())");
            sb.AppendLine("                    {");
            ...
            sb.AppendLine("            {");
            sb.AppendLine("                dataQuery.RecordCount = basicDataAccess.GetRecordCount(trans,CommandType.Text, dataQuery, dataQuery.WhereParameters);");
            ...
            sb.AppendLine("        List<" + Tbl.PascalName + "Info> returnList = new List<" + Tbl.PascalName + "Info>();");
            sb.AppendLine("        ");
            sb.AppendLine("        dataQuery.TableName = \"" + Tbl.PascalName + "\";");
            sb.AppendLine("        ");
            sb.AppendLine("        try");
            sb.AppendLine("        {");
            sb.AppendLine("            BasicDataAccess bm = new BasicDataAccess();");
            sb.AppendLine("            //开始读取数据(start reading data.)");
            sb.AppendLine("            using (IDataReader dr = bm.ExecuteReader(trans,CommandType.Text, dataQuery, dataQuery.WhereParameters))");
            sb.AppendLine("            {");
            sb.AppendLine("                while (dr.Read())");
            sb.AppendLine("                {");
            sb.AppendLine("                    returnList.Add(Create" + Tbl.PascalName + "InfoByReader(dr));");
            ...
            sb.AppendLine("            if (dataQuery.IsPageView)");
            sb.AppendLine("            {");
            sb.AppendLine("                dataQuery.RecordCount = bm.GetRecordCount(trans,CommandType.Text, dataQuery, dataQuery.WhereParameters);");
            sb.AppendLine("                dataQuery.PageCount = Convert.ToInt32(dataQuery.RecordCount / dataQuery.PageSize) + 1;");
            sb.AppendLine("            }");
            sb.AppendLine("        }");
            sb.AppendLine("        catch (Exception ex)");
