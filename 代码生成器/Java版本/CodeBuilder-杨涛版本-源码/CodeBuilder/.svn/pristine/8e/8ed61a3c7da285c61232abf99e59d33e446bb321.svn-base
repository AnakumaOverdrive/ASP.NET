...
{
    public class SqlDAL_001 : ITemplate
    {
        ...
        public IReturnCode[] GetCode()
        {
            ...
            sb.AppendLine();
            sb.AppendLine("namespace Esint." + NameSpace + ".SqlDAL");
            sb.AppendLine("{");
            ...
            sb.AppendLine("            {");
            sb.AppendLine("                base.ExecuteNonQuery(connect,operateObject);");
            //如果主键是标识列,则查询标识,并返回
            if (Tbl.PrimaryKey != null)
            {
                foreach (IColumn col in Tbl.PrimaryKey.Columns)
                {
                    if (col.IsIndentity)
                    {
                        sb.AppendLine("                 " + Tbl.CamelName + "." + col.PascalName + " = base.GetIdentity(connect);");
                        break;
                    }
                }
            }
            ...
            sb.AppendLine("                {");
            sb.AppendLine("                    dataQuery.RecordCount = basicDataAccess.GetRecordCount(connection,CommandType.Text, dataQuery, dataQuery.WhereParameters);");
