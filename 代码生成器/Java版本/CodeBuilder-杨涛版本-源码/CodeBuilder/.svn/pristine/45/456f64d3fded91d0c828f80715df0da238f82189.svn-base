...
{
    public class OracleDAL_11:ITemplate
    {
        ...
        public StringBuilder BuilderDesignerCode()
        {
            ...
            sb.AppendLine("    /// 生成模板: Esint.Template.OracleDAL.OracleDAL_11 版");
            sb.AppendLine("    ///           适用于反射工厂架构模式");
            ...
            foreach (IColumn col in Tbl.PrimaryKey.Columns)
            {
                ...
                {
                    sb.AppendLine("            if(" + Tbl.CamelName + "." + col.PascalName + " ==null)");
                    sb.AppendLine("                " + Tbl.CamelName + "." + col.PascalName + " =basicDataAccess.GetSequences(\"" + Tbl.TableName + "\", 15, \"YY\",trans);");
                }
                     }
            sb.AppendLine("            //建立SQL语句对象");
            sb.AppendLine("            OperateObject operateObject = CreateInsertObject(" + Tbl.CamelName + ");");
            ...
            for (int column = 0; column < Tbl.Columns.Count; column++)
            {
                ...
                if (col.DataType.AutoWidth)
                    sb.AppendLine("                para = new OracleParameter(\":" + col.PascalName + "\",OracleDbType." + col.DataType.DbType + ");");
                else
                    sb.AppendLine("                para = new OracleParameter(\":" + col.PascalName + "\",OracleDbType." + col.DataType.DbType + colLength + ");");

                if (col.DataType.CSharpType == "Guid")
                    sb.AppendLine("                para.Value = " + Tbl.CamelName + "." + col.PascalName + ".ToString();");
                else
            ...
            if (Tbl.PrimaryKey != null)
            {
                ...
                {
                    if (col.DataType.CSharpType == "Guid")
                        sb.AppendLine("            whereParaList.Add(new WhereParameter(\"And " + col.ColumnName + " = :" + col.PascalName + "\", \":" + col.PascalName + "\", " + Tbl.CamelName + "." + col.PascalName + ".ToString()));");
                    else
            ...
            sb.AppendLine("        /// </summary>");
            sb.AppendLine("        public void Update(" + Tbl.PascalName + "Info " + Tbl.CamelName + ", List<WhereParameter> whereParaList,IDbTransaction trans)");
            sb.AppendLine("        {");
            sb.AppendLine("            if (whereParaList == null || whereParaList.Count == 0)");
            sb.AppendLine("                throw new ArgumentException(\"根据条件更新,条件列表不能为空!\");");
            sb.AppendLine();
            ...
            foreach (IColumn col in Tbl.Columns)
            {
                ...
                if (col.IsPrimaryKey == false)
                {
                    ...
                    if (col.DataType.CSharpType == "Guid")
                        tempStr += "\r\n                para.Value = " + Tbl.CamelName + "." + col.PascalName + ".ToString();";
                    else
            ...
            tempStr += "\r\n            if (!string.IsNullOrEmpty(" + Tbl.CamelName + ".UpdateNullFields))";
            tempStr += "\r\n            {";
            tempStr += "\r\n                foreach (string fieldsName in " + Tbl.CamelName + ".UpdateNullFields.Split(','))";
            tempStr += "\r\n                {";
            ...
            sb.AppendLine("                 parameters.Add(new OracleParameter(wherePara.ParameterName, wherePara.Value.ToString()));");
            sb.AppendLine("             }");
            sb.AppendLine();
            sb.AppendLine("             //构建日志对象");
            sb.AppendLine("             OperateObject operObj = new OperateObject();");
            ...
            {
                if (col.DataType.CSharpType=="Guid")
                    sb.AppendLine("            whereParaList.Add(new WhereParameter(\"And " + col.ColumnName + " = :" + col.PascalName + "\", \":" + col.PascalName + "\", " + col.CamelName + ".ToString()));");
                else
            ...
            else
            {
                ...
                {
                    if (col.DataType.CSharpType=="Guid")
                        sb.AppendLine("            parameters[" + i++ + "].Value = " + col.CamelName + ".ToString();//" + col.Description);
                    else
            ...
            sb.AppendLine("            {");
            sb.AppendLine("                using (IDataReader dr = basicDataAccess.ExecuteReader(connection, CommandType.Text, strSql.ToString(), parameters))");
            sb.AppendLine("                {");
            ...
            sb.AppendLine("        /// <param name=\"OracleQuery\">查询参数</param>");
            sb.AppendLine("        /// <returns>List<></returns>");
            sb.AppendLine("        public List<" + Tbl.PascalName + "Info> Get" + Tbl.PascalName + "List(DataQuery dataQuery,IDbTransaction trans)");
            sb.AppendLine("        {");
            sb.AppendLine("            if (dataQuery.JoinTables.Count > 0)");
            sb.AppendLine("                throw new ArgumentException(\"得到对象列表方法,只能执行单表查询!\");");
            ...
            foreach (IColumn col in Tbl.Columns)
            {
                ...
                if (col.DataType.CSharpType.ToUpper() == "BYTE[]")
                    ...
                    sb.AppendLine("                " + Tbl.CamelName + "." + col.PascalName + " =  new Guid(Convert.ToString(dr[\"" + col.ColumnName + "\"]));");
        ...
        public StringBuilder BuilderCode()
        {
            ...
            if (Tbl.PrimaryKey == null || Tbl.PrimaryKey.Columns.Count == 0)
            {
                sb.Append("主键不能为空,生成失败~!");
                return sb;
        ...
        private bool IsAllowColLength(string colType)
        {
            ...
            string[] types = new string[]{
                "DateTime","byte[]","int","int?","long","double","double?","DateTime?"
            };
