...
function writeIframe()
{
    ...
    " onchange='parent.WebCalendar.thisYear =this.value; parent.hiddenSelect(this); pare  nt.writeCalendar();'></select>"+
    ...
    "    <table id=tableDay height=120 width=140 border=0 cellspacing=1 cellpadding=0>";
         ...
         " type=button value='&nbsp; &nbsp; 清空' onclick='parent.Clear()'></td></tr></table>"+
...
function TWebCalendar() //初始化日历的设置
{
    ...
    this.darkColor  = "#0591d5";    //控件的暗色

    this.lightColor = "#FFFFFF";    //控件的亮色

    this.btnBgColor = "#FFFFF5";    //控件的按钮背景色
    this.wordColor  = "#1a6aa5";    //控件的文字颜色

    this.wordDark   = "#b8b8b8";    //控件的暗文字颜色
    this.dayBgColor = "#e9f6fa";    //日期数字背景色

    this.todayColor = "#fcb449";    //今天在日历上的标示背景色
...
function calendar() //主调函数
{
    ...
    if  (!WebCalendar.timeShow) WebCalendar.dateReg = /^(\d{1,4})-(\d{1,2})-(\d{1,2})$/;
...
function Clear(){WebCalendar.eventSrc.value="";hiddenCalendar();};
...
var calendar_events; 
this.calendar_oldEventHandler ;
if(document.onclick)
{
    calendar_events =  'this.calendar_oldEventHandler ='+ document.onclick.toString();
...
{  if (window.event!=undefined)
     {
      ...
      if (calendar_events!=undefined) { eval(calendar_events); this.calendar_oldEventHandler();}
